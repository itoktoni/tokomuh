...
class VariantController extends Controller
{
    ...
    {
        if (self::$model == null) {
            self::$model = new VariantRepository();
        }
    ...
    {
        if (request()->isMethod('POST')) {
            $datatable = $service->setRaw(['item_variant_image'])->datatable(self::$model);
            $datatable->editColumn('item_variant_image', function ($select) {
                return Helper::createImage(Helper::getTemplate(__CLASS__) . '/thumbnail_' . $select->item_variant_image);
            });
