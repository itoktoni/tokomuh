...
class WaOrderApproved extends Command
{
    ...
    public function handle()
    {
        ...
        $order_group = OrderGroupFacades::where('sales_group_status', 3)->whereNull('sales_group_date_wa_approved_wa')->limit(1)->get();
        if ($order_group) {
            foreach ($order_group as $group_item) {
                ...
                $message = $message . '\nSub Total : ' . Helper::createRupiah($group_item->sales_group_sum_total) . '\n';
                ...
                $group_item->sales_group_date_wa_approved_wa = date('Y-m-d H:i:s');
                $group_item->save();
